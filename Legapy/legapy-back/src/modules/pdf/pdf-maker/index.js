...
const createSession = (html, pdfPath, option, callback) => {
    try {
        if (_session) {
            createPage(_session, html, pdfPath, option, callback)
        } else {
            phantom.create({
                path: phantomjs.path,
            }, (err, session) => {
...
const pdfMakerAsync = promisify((template, data, pdfPath, option, callback) => {
    try {
        ...
        if (fileExtension === 'html') {
            ...
            // console.log('Please provide data object')
                return callback(new Error('Please provide data object'))
            }

            if (!pdfPath) {
            // console.log('Please provide file path of the pdf')
                return callback(new Error('Please provide file path of the pdf'))
            }
        ...
        // console.log('Unknown file extension')
            return callback(new Error('Unknown file extension'))
        }
    } catch (e) {
        return callback(e)
    }
